...
class MarketService:
    ...
    def update_market(self, tipo: str) -> list[sqlite3.Row]:
        ...
        with self.database._get_connection() as connection:
            ...
            for asset in assets:
                ...
                change = base_change + demand_bias + mean_reversion + event_impact
    ...
    def _random_event(tipo: str) -> dict[str, object]:
        events = [
            ...
            ("Bull Market", 0.02),
            ("Bear Market", -0.02),
            ("Incentivo Governamental", 0.015),
            ("Crise Economica", -0.035),
            ("Super Alta", 0.04),
            ("Crash Financeiro", -0.06),
        ]
        name, impact = random.choices(events, weights=[50, 10, 10, 8, 10, 6, 6], k=1)[0]
        if tipo == "crypto":
            impact *= 1.25
